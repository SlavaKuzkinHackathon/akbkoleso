...
using akbkoleso.Infrastructure;
using Microsoft.EntityFrameworkCore;


namespace akbkoleso.Controllers
{
    public class AkbController : Controller
    {
        private akbkolesoContext context;
        public IProductRepository productRepository;

        public AkbController(IProductRepository repo, akbkolesoContext ctx)
        {
            context = ctx;
        ...
        public async Task<IActionResult> Index(string SearchName, string NameA, string NameB, string NameC, string NameD, string NameF, string NameG,
             string category = "Аккумуляторы", int page = 1, SortState sortOrder = SortState.RetailPriceAsc)
        {
